...
impl RadioCallQueries {
    ...
    pub async fn insert(pool: &PgPool, call: &RadioCall) -> Result<Uuid> {
        ...
        let confidence = call.transcription_confidence.map(rust_decimal::Decimal::try_from).transpose()
            .map_err(|e| Error::Database(format!("Invalid confidence value: {e}")))?;
        
        let duration = call.duration_seconds.map(rust_decimal::Decimal::try_from).transpose()
            .map_err(|e| Error::Database(format!("Invalid duration value: {e}")))?;

        let upload_ip = call.upload_ip.as_deref()
        ...
        let row = sqlx::query(query)
            ...
            .bind(call.patches.as_ref().map(|p| p.to_string()))
            .bind(call.frequencies.as_ref().map(|f| f.to_string()))
            .bind(call.sources.as_ref().map(|s| s.to_string()))
            .bind(upload_ip)
    ...
    pub async fn update_transcription_status(
        ...
        let confidence_decimal = confidence.map(rust_decimal::Decimal::try_from).transpose()
            .map_err(|e| Error::Database(format!("Invalid confidence value: {e}")))?;
    ...
    pub async fn get_transcription_stats(pool: &PgPool) -> Result<TranscriptionStats> {
        ...
        Ok(TranscriptionStats {
            ...
            avg_confidence: row.get::<Option<rust_decimal::Decimal>, _>("avg_confidence")
...
impl UploadLogQueries {
    ...
    pub async fn get_upload_stats(pool: &PgPool) -> Result<UploadStats> {
        ...
        Ok(UploadStats {
            ...
            avg_processing_time: row.get::<Option<rust_decimal::Decimal>, _>("avg_processing_time")
                .map(|d| d.to_string().parse::<f64>().unwrap_or(0.0)),
            total_bytes_uploaded: row.get::<Option<i64>, _>("total_bytes_uploaded")
                .unwrap_or(0),
        })
    }
}
...
mod tests {
    ...
    async fn test_radio_call_count_by_system() {
        ...
        for i in 0..5 {
            let mut call = RadioCall::default();
            call.system_id = "count_test_system".to_string();
            call.talkgroup_id = Some(i);
            RadioCallQueries::insert(&pool, &call).await.unwrap();
        }
        
        let count = RadioCallQueries::count_by_system(&pool, "count_test_system").await.unwrap();
        assert_eq!(count, 5);
    }
    ...
    async fn test_transcription_status_update() {
        ...
        let mut call = RadioCall::default();
        call.system_id = "transcription_test".to_string();
        ...
        RadioCallQueries::update_transcription_status(
            &pool, id, "processing", None, None, None
        ).await.unwrap();
        ...
        // Update to completed
        RadioCallQueries::update_transcription_status(
            &pool, id, "completed", Some("Test transcription"), Some(0.95), None
        ).await.unwrap();
        
        let completed = RadioCallQueries::find_by_id(&pool, id).await.unwrap();
        assert_eq!(completed.transcription_status, Some("completed".to_string()));
        assert_eq!(completed.transcription_text, Some("Test transcription".to_string()));
    ...
    async fn test_system_stats_upsert() {
        ...
        let retrieved = SystemStatsQueries::get_by_system_id(&pool, "upsert_test").await.unwrap();
        ...
        SystemStatsQueries::upsert(&pool, &updated_stats).await.unwrap();
        
        let updated_retrieved = SystemStatsQueries::get_by_system_id(&pool, "upsert_test").await.unwrap();
...
pub async fn list_radio_calls_filtered(
    ...
    if let Some(system) = system_id {
        RadioCallQueries::find_by_system(pool, system, limit, offset).await
    } else {
...
pub async fn count_radio_calls_filtered(
    pool: &PgPool,
    system_id: Option<&str>,
    talkgroup_id: Option<i32>,
    from_date: Option<chrono::DateTime<chrono::Utc>>,
...
pub async fn get_top_systems(pool: &PgPool, limit: i64) -> Result<Vec<(String, i64)>> {
    ...
    Ok(rows.into_iter().map(|row| (row.get("system_id"), row.get("count"))).collect())
}

/// Count system calls since a given time
pub async fn count_system_calls_since(
    pool: &PgPool,
    system_id: &str,
    hours: i32,
) -> Result<i64> {
...
pub async fn update_system_stats(pool: &PgPool, system_id: &str, system_label: Option<String>) -> Result<()> {
