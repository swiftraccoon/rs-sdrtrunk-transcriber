...
/// Result type alias for storage query operations.
...
impl RadioCallQueries {
    ...
    pub async fn insert(pool: &PgPool, call: &RadioCallDb) -> Result<Uuid> {
        let query = r"
        ...
        let row = sqlx::query(query)
            ...
            .bind(&call.audio_content_type)
            .bind(call.duration_seconds)
            .bind(&call.transcription_text)
            .bind(call.transcription_confidence)
            .bind(&call.transcription_language)
            .bind(&call.transcription_status)
    ...
    pub async fn count_by_system(pool: &PgPool, system_id: &str) -> Result<i64> {
        ...
        let row = sqlx::query(query).bind(system_id).fetch_one(pool).await?;
    ...
    pub async fn delete_older_than(
        ...
        let result = sqlx::query(query).bind(older_than).execute(pool).await?;
...
impl SystemStatsQueries {
    ...
    pub async fn update_activity(pool: &PgPool, system_id: &str) -> Result<()> {
        ...
        let _result = sqlx::query(query).bind(system_id).execute(pool).await?;
...
pub struct CreateApiKeyParams<'a> {
    ...
    /// Optional list of allowed systems
...
impl ApiKeyQueries {
    ...
    pub async fn update_usage(pool: &PgPool, key_id: &str) -> Result<()> {
        ...
        let _result = sqlx::query(query).bind(key_id).execute(pool).await?;
    ...
    ///
    /// Returns an error if the database query fails.
    pub async fn create(pool: &PgPool, params: CreateApiKeyParams<'_>) -> Result<ApiKeyDb> {
        let query = r"
            INSERT INTO api_keys (
                id, key_hash, description, expires_at, allowed_ips, allowed_systems, active
            ) VALUES (
                gen_random_uuid()::text, $1, $2, $3, $4, $5, true
            )
            RETURNING *
        ";
        ...
        sqlx::query_as::<_, ApiKeyDb>(query)
            ...
            .bind(params.allowed_systems)
            .fetch_one(pool)
            .await
            .map_err(StorageError::from)
    }

    /// Delete an API key (soft delete by marking as inactive)
    ///
    /// # Errors
    ///
    /// Returns an error if the database query fails.
    pub async fn delete(pool: &PgPool, key_id: &str) -> Result<()> {
        let query = r"
            UPDATE api_keys
            SET active = false
            WHERE id = $1
    ...
    /// Get API key by ID (including inactive keys for admin viewing)
    ///
    /// # Errors
    ///
    /// Returns an error if the database query fails or key is not found.
    pub async fn find_by_id(pool: &PgPool, key_id: &str) -> Result<ApiKeyDb> {
        let query = r"
            SELECT * FROM api_keys
            WHERE id = $1
        ";
...
///
/// Returns an error if the database query fails.
pub async fn insert_radio_call(pool: &PgPool, call: &RadioCallDb) -> Result<Uuid> {
...
pub async fn get_radio_call(pool: &PgPool, id: Uuid) -> Result<Option<RadioCallDb>> {
    match RadioCallQueries::find_by_id(pool, id).await {
        ...
        Err(StorageError::Query(ref msg)) if msg.contains("no rows returned") => Ok(None),
        Err(e) => Err(e),
    }
}
...
/// Count radio calls with filtering (now properly applies all filters!)
///
/// # Errors
///
/// Returns an error if the database query fails.
pub async fn count_radio_calls_filtered(pool: &PgPool, filter: RadioCallFilter<'_>) -> Result<i64> {
    // Build dynamic WHERE clause matching the same logic as find_by_system/find_all_with_filters
    let mut conditions = Vec::new();
    let mut param_count = 0;

    // System ID filter
    if let Some(_system_id) = filter.system_id {
        param_count += 1;
        conditions.push(format!("system_id = ${param_count}"));
    }

    // Talkgroup filter
    if let Some(_talkgroup_id) = filter.talkgroup_id {
        param_count += 1;
        conditions.push(format!("talkgroup_id = ${param_count}"));
    }

    // Transcription status filter
    if let Some(_status) = filter.transcription_status {
        param_count += 1;
        conditions.push(format!("transcription_status = ${param_count}"));
    }
    ...
    let query_str = format!("SELECT COUNT(*) FROM radio_calls {where_clause}");
    ...
    query.fetch_one(pool).await.map_err(StorageError::from)
}
...
/// # Panics
///
/// Panics if the fallback system ID `"unknown"` fails validation (should never happen).
pub async fn update_system_stats(
    ...
    let stats = SystemStatsDb {
        id: Uuid::new_v4(),
        system_id: sdrtrunk_types::SystemId::new(system_id)
            .or_else(|_| sdrtrunk_types::SystemId::new("unknown"))
            .map_err(|e| StorageError::Serialization(format!("Invalid system ID: {e}")))?,
        system_label,
        total_calls: Some(1),
        calls_today: Some(1),
...
mod tests {
    ...
    // Test data helpers
    fn create_test_radio_call(system_id: &str, talkgroup_id: Option<i32>) -> RadioCallDb {
        RadioCallDb {
            id: Uuid::new_v4(),
            ...
            frequency: Some(Frequency::new(154_000_000).unwrap()),
            talkgroup_id: talkgroup_id.and_then(|id| TalkgroupId::new(id).ok()),
            talkgroup_label: Some("Test TG".to_string()),
            talkgroup_group: Some("Public Safety".to_string()),
            talkgroup_tag: Some("Police".to_string()),
            source_radio_id: Some(RadioId::new(12345).unwrap()),
            ...
            duration_seconds: Some(rust_decimal::Decimal::try_from(30.5).unwrap()),
            transcription_text: Some("Test transcription".to_string()),
            transcription_confidence: Some(rust_decimal::Decimal::try_from(0.95).unwrap()),
            ...
            patches: Some(
                serde_json::to_string(&serde_json::json!([{"id": 1, "name": "Patch1"}])).unwrap(),
            ),
            frequencies: Some(serde_json::to_string(&serde_json::json!([154_000_000])).unwrap()),
            sources: Some(
                serde_json::to_string(&serde_json::json!([{"id": 1, "name": "Source1"}])).unwrap(),
            ),
            upload_ip: Some(sqlx::types::ipnetwork::IpNetwork::from(IpAddr::from([
                127, 0, 0, 1,
            ]))),
    ...
    async fn test_radio_call_insert_and_find() -> Result<()> {
        ...
        let call = create_test_radio_call("test_system", Some(12345));

        let id = RadioCallQueries::insert(&pool, &call).await?;
        assert!(!id.is_nil());

        let retrieved = RadioCallQueries::find_by_id(&pool, id).await?;
        assert_eq!(retrieved.system_id.as_str(), "test_system");
        assert_eq!(retrieved.talkgroup_id.map(TalkgroupId::as_i32), Some(12345));
    ...
    async fn test_radio_call_count_by_system() -> Result<()> {
        ...
        // Insert test calls
        for i in 0..5 {
            let call = create_test_radio_call(&unique_system, Some(i));
            RadioCallQueries::insert(&pool, &call).await?;
        }
    ...
    async fn test_transcription_status_update() -> Result<()> {
        ...
        let call = create_test_radio_call("transcription_test", None);
    ...
    async fn test_system_stats_upsert() -> Result<()> {
        ...
        let stats = SystemStatsDb {
            id: Uuid::new_v4(),
            system_id: SystemId::new("upsert_test").unwrap(),
            system_label: Some("Test System".to_string()),
        ...
        assert_eq!(retrieved.system_id.as_str(), "upsert_test");
    ...
    async fn test_upload_log_operations() -> Result<()> {
        ...
        let log = UploadLogDb {
            ...
            client_ip: "127.0.0.1"
                .parse::<IpAddr>()
                .map_err(|e| StorageError::Serialization(format!("client_ip: {e}")))?
                .into(),
    ...
    async fn test_transcription_update_with_error() -> Result<()> {
        ...
        let call = create_test_radio_call("error_test", None);
    ...
    async fn test_radio_call_with_all_optional_fields() -> Result<()> {
        ...
        let call = RadioCallDb {
            id: Uuid::new_v4(),
            created_at: chrono::Utc::now(),
            call_timestamp: chrono::Utc::now(),
            system_id: SystemId::new(system_id.clone()).unwrap(),
            system_label: Some("Full Test System".to_string()),
            frequency: Some(Frequency::new(460_125_000).unwrap()),
            talkgroup_id: Some(TalkgroupId::new(98765).unwrap()),
            talkgroup_label: Some("Test Talkgroup".to_string()),
            talkgroup_group: Some("Emergency".to_string()),
            talkgroup_tag: Some("Fire".to_string()),
            source_radio_id: Some(RadioId::new(54321).unwrap()),
            ...
            audio_content_type: None,
            duration_seconds: Some(rust_decimal::Decimal::try_from(125.75).unwrap()),
            transcription_text: Some("This is a full test transcription".to_string()),
            transcription_confidence: Some(rust_decimal::Decimal::try_from(0.98).unwrap()),
            transcription_language: None,
            transcription_status: Some("completed".to_string()),
            ...
            patches: Some(
                serde_json::to_string(&serde_json::json!([{"id": 1, "name": "Patch A"}])).unwrap(),
            ),
            frequencies: Some(
                serde_json::to_string(&serde_json::json!([460_125_000, 460_150_000])).unwrap(),
            ),
            sources: Some(
                serde_json::to_string(&serde_json::json!([{"id": 1, "name": "Site 1"}])).unwrap(),
            ),
            upload_ip: Some(sqlx::types::ipnetwork::IpNetwork::from(
        ...
        let retrieved = RadioCallQueries::find_by_id(&pool, id).await?;
        assert_eq!(retrieved.system_id.as_str(), system_id);
        assert_eq!(retrieved.talkgroup_id.map(TalkgroupId::as_i32), Some(98765));
    ...
    fn test_error_conversions() {
        // Test that sqlx errors are properly converted to StorageError
        let storage_error = StorageError::from(sqlx::Error::RowNotFound);
    ...
    async fn test_get_transcription_stats() -> Result<()> {
            ...
            .enumerate()
        {
            ...
            call.transcription_status = Some((*status).to_string());
            call.transcription_confidence = if *status == "completed" {
                Some(rust_decimal::Decimal::try_from(0.85).unwrap())
            } else {
                None
            };
            RadioCallQueries::insert(&pool, &call).await?;
    ...
    async fn test_transcription_stats() -> Result<()> {
        ...
        let confidences: [Option<rust_decimal::Decimal>; 4] = [
            Some(rust_decimal::Decimal::try_from(0.95).unwrap()),
            None,
            None,
            None,
        ];
    ...
    async fn test_system_stats_get_all() -> Result<()> {
        ...
        for stat in &all_stats {
            if !prev_id.is_empty() {
                assert!(stat.system_id.as_str() >= prev_id.as_str());
            }
            prev_id = stat.system_id.as_str().to_string();
        }
    ...
    async fn test_system_stats_activity_update() -> Result<()> {
        ...
        assert_eq!(stats2.system_id.as_str(), system_id);
    ...
    async fn test_wrapper_functions() -> Result<()> {
        ...
        assert_eq!(retrieved.unwrap().system_id.as_str(), system_id);
    ...
    async fn test_system_operations() -> Result<()> {
        ...
        assert_eq!(stats.system_id.as_str(), system_id);
    ...
    fn test_error_handling_scenarios() {
        // Test various sqlx error mappings via StorageError
        let mapped_error = StorageError::from(sqlx::Error::RowNotFound);

        if let StorageError::NotFound { entity, id } = mapped_error {
            assert_eq!(entity, "unknown");
            assert_eq!(id, "unknown");
        } else {
        ...
        // Test StorageError to AppError bridge conversion
        let storage_err = StorageError::NotFound {
            entity: "ApiKey".to_string(),
            id: "test_key".to_string(),
        };
        let core_err: Error = storage_err.into();
        match core_err {
            Error::Database(msg) => assert!(msg.contains("ApiKey")),
            _ => panic!("Expected Database error from bridge conversion"),
        }
    }
    ...
    fn test_helper_functions() {
        ...
        assert_eq!(call.system_id.as_str(), "test_sys");
        assert_eq!(call.talkgroup_id.map(TalkgroupId::as_i32), Some(789));
        ...
        assert_eq!(stats.system_id.as_str(), "helper_test");
    ...
    fn test_ip_address_conversions() {
        ...
        let mapped_ipv4: IpAddr = "::ffff:192.168.1.1".parse().unwrap();
    ...
    fn test_invalid_ip_string_parsing() {
        ...
        for invalid_ip in invalid_ips {
            let result = invalid_ip.parse::<IpAddr>();
            assert!(result.is_err());
        }
    }
    ...
    #[test]
    fn test_radio_call_db_default_like_values() {
        // Test RadioCallDb with minimal fields
        let call = create_test_radio_call("default_test", None);

        assert!(!call.id.is_nil());
        assert!(call.talkgroup_id.is_none());
        assert!(call.audio_size_bytes.is_some());
        assert!(call.transcription_text.is_some());
    }

    #[test]
    fn test_radio_call_db_with_boundary_values() {
        // Test RadioCallDb with boundary values for typed fields
        let call = RadioCallDb {
            id: Uuid::new_v4(),
            created_at: chrono::Utc::now(),
            call_timestamp: chrono::Utc::now(),
            system_id: SystemId::new("boundary_test").unwrap(),
            system_label: Some("y".repeat(255)),
            frequency: Some(Frequency::new(999_999_999).unwrap()),
            talkgroup_id: Some(TalkgroupId::new(65535).unwrap()),
            talkgroup_label: Some("z".repeat(255)),
            talkgroup_group: Some("group".repeat(50)),
            talkgroup_tag: Some("tag".repeat(50)),
            source_radio_id: Some(RadioId::new(999_999).unwrap()),
            ...
            audio_content_type: Some("audio/wav".to_string()),
            duration_seconds: Some(rust_decimal::Decimal::try_from(86400.0).unwrap()),
            transcription_text: Some("text".repeat(1000)),
            transcription_confidence: Some(rust_decimal::Decimal::try_from(1.0).unwrap()),
            transcription_language: Some("en".to_string()),
            transcription_status: Some("completed".to_string()),
            speaker_segments: Some(serde_json::json!([{"test": "data"}])),
            speaker_count: Some(i32::MAX),
            patches: Some(serde_json::to_string(&serde_json::json!([{"patch": "data"}])).unwrap()),
            frequencies: Some(serde_json::to_string(&serde_json::json!([999_999_999])).unwrap()),
            sources: Some(serde_json::to_string(&serde_json::json!([{"source": "data"}])).unwrap()),
            upload_ip: Some(sqlx::types::ipnetwork::IpNetwork::from(
        ...
        assert_eq!(call.system_id.as_str(), "boundary_test");
        assert_eq!(call.audio_size_bytes, Some(i64::MAX));
        assert_eq!(call.speaker_count, Some(i32::MAX));
    }
    ...
    fn test_upload_log_db_field_validation() {
        ...
        let log = UploadLogDb {
            id: Uuid::new_v4(),
            timestamp: chrono::Utc::now(),
            client_ip: "192.168.1.1".parse::<IpAddr>().unwrap().into(),
    ...
    fn test_system_stats_db_field_validation() {
        ...
        let stats = SystemStatsDb {
            id: Uuid::new_v4(),
            system_id: SystemId::new("comprehensive_system").unwrap(),
        ...
        assert_eq!(stats.system_id.as_str(), "comprehensive_system");
    ...
    fn test_wrapper_functions_logic() {
        ...
        // Test the error matching logic in get_radio_call (now uses StorageError)
        let db_not_found_error = StorageError::Query(
            "no rows returned by a query that expected to return at least one row".to_string(),
        );
        let other_db_error = StorageError::Query("connection failed".to_string());

        // Verify error message matching
        if let StorageError::Query(ref msg) = db_not_found_error {
            assert!(msg.contains("no rows returned"));
        }

        if let StorageError::Query(ref msg) = other_db_error {
            assert!(!msg.contains("no rows returned"));
        }
    }
    ...
    fn test_system_stats_creation() {
        ...
        assert_eq!(stats.system_id.as_str(), "test_system_123");
    ...
    fn test_radio_call_creation_variations() {
        ...
        assert_eq!(minimal_call.system_id.as_str(), "minimal");
        assert!(minimal_call.talkgroup_id.is_none());

        // Test with talkgroup ID
        let with_talkgroup = create_test_radio_call("with_tg", Some(999));
        assert_eq!(with_talkgroup.system_id.as_str(), "with_tg");
        assert_eq!(
            with_talkgroup.talkgroup_id.map(TalkgroupId::as_i32),
            Some(999)
        );

        // Test all fields are properly set
        assert!(!minimal_call.id.is_nil());
    ...
    fn test_ip_address_parsing_extended() {
        ...
        for (ip_str, should_parse) in test_cases {
            let parse_result = ip_str.parse::<IpAddr>();
    ...
    fn test_error_message_formatting() {
        ...
        for message in test_messages {
            // Test that messages can be formatted into StorageError
            let error = StorageError::Query(message.to_string());
    ...
    fn test_upload_log_params_ip_address_parsing() {
        ...
        for addr_str in ipv4_addresses {
            let ip = addr_str.parse::<IpAddr>().unwrap();
            let params = UploadLogParams {
                client_ip: ip,
            ...
            match params.client_ip {
                IpAddr::V4(_) => {}
                IpAddr::V6(_) => panic!("Expected IPv4 for {addr_str}"),
            }
        }
        ...
        for addr_str in ipv6_addresses {
            ...
            match params.client_ip {
                IpAddr::V4(_) => panic!("Expected IPv6 for {addr_str}"),
                IpAddr::V6(_) => {} // IPv6 as expected
            }
        }
    }
